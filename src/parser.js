...
/**
 * Describe an ASTNode with name, value and full position range.
 */
export class ASTNode {
  /**
   * Construct a new node wit relevant details.
   * @param {string} name of that node type.
   * @param {*} value of the parsed node.
...
/**
 * A Parser to interpret tokens into a logic.
 */
export default class Parser {
   ...
   * @return {boolean} whether that check succeeded.
   */
  peekCheck(name) {
  ...
  /**
   * Increments the current position in the tokens list and reads the element.
   * This should be called after {@link this.peekCheck} to consume the
   * peeked token.
   * @return {TokenMatch} peeked token
   */
  consumeToken() {
    this._position++;
  ...
  /**
   * This consumes the peeked token with {@link this.consumeToken} and throws
   * an error including the token information and position.
   * @throws {Error} with current tokens details.
   ...
   * @throws {Error} with {@link Parser.throwTokenError} on unexpected token.
   * @param {Array<TokenMatch>} tokens generated by the Tokenizer.
   * @return {Array<ASTNode>} list of ast nodes in the root structure.
   */
  parse(tokens) {
    this.reset();
  ...
  /**
   * Parse a full function call with all parameters and function block.
   * @throws {Error} with {@link Parser.throwTokenError} on unexpected token.
   * @param {ASTNode} dataReference that started this function call parse.
   * @return {ASTNode} with function call information.
   */
    ...
    while (this._position < this._tokens.length) {
      if (this.peekCheck(TokenNames.DATA_BOOLEAN) && expectValue) {
        ...
        this.throwTokenError(
          expectValue ?
            'function parameter' :
            'function closing or list separator');
      }
    }
   ...
   * @throws {Error} with {@link Parser.throwTokenError} on unexpected token.
   * @param {ASTNode} openBlock that started this block.
   * @return {ASTNode} function block node.
   */
